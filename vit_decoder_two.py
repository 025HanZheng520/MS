...
class DecoderBlock(nn.Module):
    def __init__(self, dim, num_heads, dim_q=None, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.conv = nn.Sequential(nn.Conv2d(dim//2, dim, kernel_size=3, stride=2, padding=1, bias=False),
    ...
    def forward(self, x_1, x_2):
        #x_1 = x[:, :self.img_dim, :]
        #x_2 = x[:, self.img_dim:, :]
        #print("decoder:"+str(x_1.size())+str( x_2.size()))
        b, f,c = x_2.shape
        x_2 = x_2.view(b*f, c, 1,1)
        x_2 = self.conv(x_2)
        #print(x_2.size())
        x_2 = x_2.view(b , f, -1)
        ...
        #print(x_frame.size())
        #print(x_frame.size(), x_heatmap.size())
        #x = torch.cat((x_frame, x_heatmap), dim=1) #torch.Size([10, 34, 256])
        x = torch.stack((x_frame, x_heatmap), dim=1)
        x =torch.mean(x, dim=1) #torch.Size([10, 17, 256])
        #print(x.size())
...
class decoder_fuser(nn.Module):
    ...
    def forward(self, x_frame, x_heatmap):
        #print(x_frame.size(), x_heatmap.size())
        x_frame = self.pos_drop(x_frame+self.pos_embed) #torch.Size([10, 16, 256])
        #x_comb = torch.cat((x_frame, x_heatmap), dim=1)

        for _layer in self.model:
            x_comb = _layer(x_frame, x_heatmap)
